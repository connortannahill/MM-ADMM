...
void Mesh<D>::buildMatrix() {
    ...
    try {
        ...
        matrixBuilder = new MatrixStruc(D*Vp->rows() /*Number of unknowns*/, 0 /*Add non-zeros to diagonals*/);
...
double Mesh<D>::computeEnergy(Eigen::VectorXd &x) {
    ...
    for (int i = 0; i < F->rows(); i++) {
        ...
        Ih += computeBlockGrad(i, x_i, xi_i, gradSimp, *mapEvaluator, false, false, Igt);
    }
...
template <int D>
double Mesh<D>::eulerStepMod(Eigen::VectorXd &x, Eigen::VectorXd &grad) {
    Eigen::Vector<double, D*(D+1)> x_i;
    Eigen::Vector<double, D*(D+1)> xi_i;
    Eigen::Vector<double, D*(D+1)> gradSimp;
    Eigen::Vector<double, D> pnt;
    ...
    for (int i = 0; i < F->rows(); i++) {
        ...
        Ihorig += I_wx->blockGrad(i, x_i, xi_i, gradSimp, *mapEvaluator, true, false, Igt);
...
template <int D>
double Mesh<D>::eulerGrad(Eigen::VectorXd &x, Eigen::VectorXd &grad) {
    Eigen::Vector<double, D*(D+1)> x_i;
    Eigen::Vector<double, D*(D+1)> xi_i;
    Eigen::Vector<double, D*(D+1)> gradSimp;
    Eigen::Vector<double, D> pnt;
    ...
    for (int i = 0; i < F->rows(); i++) {
        ...
        // Compute the local gradient
        Ihorig += computeBlockGrad(i, x_i, xi_i, gradSimp, *mapEvaluator, true, false, Igt);
...
template <int D>
double Mesh<D>::eulerStep(double dt, Eigen::VectorXd &x, Eigen::VectorXd &grad) {
    Eigen::Vector<double, D*(D+1)> x_i;
    Eigen::Vector<double, D*(D+1)> xi_i;
    Eigen::Vector<double, D> pnt;
    ...
    // for (int pntId = 0; pntId < Vp->rows(); pntId++) {
    //     if (boundaryMask->at(pntId) == NodeType::BOUNDARY_FREE) {
    //         Eigen::Vector<double, D> xTemp(x.segment(D*pntId, D));
    //         projectOntoBoundary(pntId, xTemp);
...
template <int D>
double Mesh<D>::predictX(double dt, int steps, Eigen::VectorXd &xPrev, Eigen::VectorXd &x, Eigen::VectorXd &xBar) {
    double dtNew = 0;
    double Ihorig = 0;
    ...
    if (gradUse) {
        ...
        if (steps <= 2) {
            ...
            // Ihorig = computeEnergy(x);
            // cout << "diff = " << (*grad - (x - xPrev)/dt).norm() << endl;
...
template <int D>
double Mesh<D>::computeBlockGrad(int zId, Eigen::Vector<double, D*(D+1)> &z,
    ...
    double Ix = I_wx->blockGrad(zId, z, xi, grad, interp, computeGrad, regularize, Igt);

    for (int i = 0; i < D+1; i++) {
        int pntId = (*F)(zId, i);

        if (boundaryMask->at(pntId) == NodeType::BOUNDARY_FIXED) {
            grad.segment(D*i, D) = Eigen::Vector<double, D>::Zero(D);
        }
    }
...
double Mesh<D>::bfgsOptSimplex(int zId, Eigen::Vector<double, D*(D+1)> &z,
    ...
    double Igt;
    Ix = computeBlockGrad(zId, z, xi, Gk, *mapEvaluator, true, true, Igt);
    double Ihsave = Igt;
    if (!hessComputed) {
        zPurt = z;
        for (int i = 0; i < D*(D+1); i++) {
            ...
            computeBlockGrad(zId, zPurt, xi, Gkp1, *mapEvaluator, true, true, Igt);
    ...
    for (iter = 0; iter < nIter; iter++) {
        ...
        Ix = computeBlockGrad(zId, z, xi, Gkp1, *mapEvaluator, true, true, Igt);
...
double Mesh<D>::newtonOptSimplex(int zId, Eigen::Vector<double, D*(D+1)> &z,
    ...
    for (int iters = 0; iters < nIter; iters++) {
        Ix = computeBlockGrad(zId, z, xi, gradZ, *mapEvaluator, true, true, Igt);
        ...
        if (iters == 0) {
            for (int i = 0; i < D*(D+1); i++) {
                ...
                // Compute gradient at purturbed point
                computeBlockGrad(zId, zPurt, xi, gradZPurt, *mapEvaluator, true, true, Igt);
...
double Mesh<D>::prox(double dt, Eigen::VectorXd &x, Eigen::VectorXd &DXpU, Eigen::VectorXd &z, double tol) {
    ...
    *this->DXpU = DXpU;
    ...
    for (int i = 0; i < F->rows(); i++) {
        ...
        (*Ih)(i) = bfgsOptSimplex(i, z_i, xi_i, 100, tol/100, hessComputed);
...
void Mesh<D>::FSubJac(double dt, int pntId, Eigen::VectorXd &x, Eigen::VectorXd &grad) {
    ...
    for (auto sId = simplexIds.begin(); sId != simplexIds.end(); ++sId) {
        ...
        // Compute the Hessian
        I_wx->blockGrad(*sId, xLoc, xiLoc, Gk, *mapEvaluator, true, false, Igt);

        // Compute purturbation
        xPurt = xLoc;
        
        if (boundaryMask->at(pntId) == BOUNDARY_FIXED) {
            derivs.setZero();

            for (int r = 0; r < D; r++) {
                for (int c = D*off; c < D*(off+1); c++) {
                    if (r == c) {
                        derivs(r, c) = 1.0;
                    }
                }
            ...
            for (int i = 0; i < D; i++) {
                ...
                // Compute gradient at purturbed point
                I_wx->blockGrad(*sId, xPurt, xiLoc, Gkp1, *mapEvaluator, true, false, Igt);
...
double Mesh<D>::backwardsEulerStep(double dt, Eigen::VectorXd &x, Eigen::VectorXd &grad, double tol) {
    ...
    *xn = x;
    ...
    const double SAFETY_FAC = 1.0/10.0;

    // Compute initial guess
    double Ih;
    Ih = eulerStepMod(x, grad);
    x -= (dt/tau)*grad;
